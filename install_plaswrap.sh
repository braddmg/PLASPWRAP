...
if env_exists "$ENV_NAME" && (( FORCE == 0 )); then
  ...
  if command -v wget >/dev/null 2>&1; then
    wget -q --no-check-certificate -O plaswrap.tar.gz https://github.com/braddmg/PLASWRAP/releases/download/v0.1.1/plaswrap-0.1.1-linux-x86_64.tar.gz
  else
    curl -sL -o plaswrap.tar.gz https://github.com/braddmg/PLASWRAP/releases/download/v0.1.1/plaswrap-0.1.1-linux-x86_64.tar.gz
  fi
